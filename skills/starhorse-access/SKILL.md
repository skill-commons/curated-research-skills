...
description: Access StarHorse SHboost and SH21 catalog products.
version: 2.0.3
...
Create an isolated CPython 3.12 environment:

```bash
python3.12 -m venv .venv
.venv/bin/python -m pip install \
  'fsspec==2026.6.0' 'pandas==3.0.3' 'pyarrow==25.0.0' \
  'pyvo==1.9.1' 'requests==2.34.2'
...
Run Python examples with `.venv/bin/python`. Public SHboost access requires no
credentials. Supply a Gaia@AIP token only for queries that the service does not expose
anonymously; never place it in the skill or a committed script.
...
```
~190 MB, 1,701,553 rows × 37 columns. Columns: `source_id`, `xgb_{av,logteff,logg,met,mass}` point estimates + `xgbdist_*_mean/std` posteriors, `dist`/`dist_lower`/`dist_upper`/`dist_flag`, `bprp0`, `mg0`, `xg/yg/zg/rg` + `v*g` velocities, string flags. **No `*50` percentile columns here (those are SH21 names); `xgb_logteff` is log10(Teff).** See the full [`references/schema.md`](references/schema.md).
...
- Full schemas re-validated 2026-07-20 against TAP_SCHEMA + the SHboost Parquet
  footer → [`references/schema.md`](references/schema.md) (SHboost column list corrected: `xgb_*` naming,
  no `*50` columns).
