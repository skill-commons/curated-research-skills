...
Use Python 3.12 in an isolated environment. The tested dependency versions are listed
below.
...
Inspect the endpoint and query before execution; begin with a tiny `TOP 1` probe.

For a reusable bounded probe that records its query, preview, cache, and provenance, run
the bundled [`scripts/tap_smoke_query.py`](scripts/tap_smoke_query.py):

```bash
.venv/bin/python scripts/tap_smoke_query.py \
  --endpoint https://www.rave-survey.org/tap/ \
  --query 'SELECT TOP 1 * FROM ravedr6.dr6_x_gaiaedr3' \
  --out tap_probe
```
...
- [ ] If REANA was requested and is available, the packaged scientific task runs there.
